...
export class TileGeometryCreator {
    ...
    createTextElements(
        ...
        if (decodedTile.textPathGeometries !== undefined) {
            ...
            for (const textPath of textPathGeometries) {
                ...
                // Make sorting stable.
                const priority = getNumberPropertyValueSafe(
                    technique.priority,
                    0,
                    displayZoomLevel
                );
        ...
        for (const srcGeometry of decodedTile.geometries) {
            ...
            for (let groupIndex = 0; groupIndex < groupCount; ) {
                ...
                const renderDepthPrePass =
                    isExtrudedPolygonTechnique(technique) &&
                    isRenderDepthPrePassEnabled(technique, displayZoomLevel);
    ...
    private getPolygonFadingParams(
        ...
        const lineFadeNear = getNumberPropertyValueSafe(
            technique.lineFadeNear,
            fadeNear,
            displayZoomLevel
        );
        const lineFadeFar = getNumberPropertyValueSafe(
