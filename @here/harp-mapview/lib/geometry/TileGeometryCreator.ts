...
import { EdgeLengthGeometrySubdivisionModifier, SubdivisionMode } from "@here/harp-geometry/lib/EdgeLengthGeometrySubdivisionModifier";
    ...
    createGroundPlane(
        ...
        function moveTileCenter(geom: THREE.BufferGeometry) {
            ...
            for (let i = 0; i < attr.array.length; i += 3) {
                tmpV.set(attr.array[i], attr.array[i + 1], attr.array[i + 2]);
                projection.reprojectPoint(sourceProjection, tmpV, tmpV);
                tmpV.sub(tile.center);
                (attr.array as Float32Array)[i] = tmpV.x;
                (attr.array as Float32Array)[i + 1] = tmpV.y;
                (attr.array as Float32Array)[i + 2] = tmpV.z;
            }
